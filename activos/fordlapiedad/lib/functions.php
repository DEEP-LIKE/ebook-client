...
function read_json($path = './json/ford.json') {
    ...
    if (!$json) {
        ...
        // Se agrega la URL del sitio en el JSON, si no existe
    ...
    // Si el JSON se lee desde el archivo y no tiene la propiedad site_url, la añadimos.
    if (isset($decodedJson->title) && !isset($decodedJson->title->site_url)) {
        $pathArray = explode('/', dirname(__FILE__));
        $folderName = $pathArray[count($pathArray)-2];
        $decodedJson->title->site_url = 'https://' . $folderName . '.ebookford.com';
    }
...
function get_title() {
    $json = read_json();
    return isset($json->title) ? $json->title : (object) ['title' => '', 'id' => '', 'site_url' => '', 'opengraph' => ''];
}
// header
function get_header() {
    $json = read_json();
